...
class PDController(Node):
    def __init__(self):
        ...
        self.sub_bumper = self.create_subscription(
            Bool, "/bumper_contact", self.onBumper, 10)
    ...
    def onBumper(self, msg):
        self.isBackOff = 1

        self.create_timer(2, lambda: setattr(self, "isBackOff", 0))
    ...
    def onOdom(self, odom_msg):
        ...
        if (self.isBackOff):
            self.vel_cmd_msg.linear.x = -self.vel_cmd[0]
            self.vel_cmd_msg.angular.z = -2 * self.vel_cmd[1] + 0.1
